...
8. [Putting It Together: The Hybrid Architecture](#8-putting-it-together-the-hybrid-architecture)
...
- **Discrete layers** are like taking snapshots of a movie at fixed frames
- **Continuous depth** is like having the full movie, where you can pause anywhere
...
For each token, we compute how relevant every other token is (by comparing queries to keys), then collect information (by weighting values accordingly).
...
$$\text{Attention}(X) = \text{softmax}\left(\frac{QK^\top}{\sqrt{d}}\right) V$$
...
Attention(X) = softmax(Q · K^T / √d) · V
```

The division by $\sqrt{d}$ (`√d`) prevents the dot products from growing too large as dimension increases, which would push softmax into saturation (all attention on one token).

### Causal Masking

For language modeling, token 5 shouldn't see tokens 6, 7, 8... (that would be cheating—using the future to predict the future). The causal mask $M$ sets attention weights to zero for future positions:
...
- **Time τ**: The vector field can behave differently at different depths. Early processing ($\tau \approx 0$) might differ from late processing ($\tau \approx 1$).
- **Control u**: An external signal that biases the dynamics (more on this in Section 7).

### Solving the ODE

We use Euler's method with $m$ steps:
...
More steps = better approximation to the true continuous dynamics, but more compute.
...
## 8. Putting It Together: The Hybrid Architecture
...
1. **Transformers stack discrete layers**, each refining the representation via residual connections.

2. **Residual updates are Euler steps** of an implicit ODE: $H_{n+1} = H_n + F(H_n)$ (`H_{n+1} = H_n + F(H_n)`).

3. **We make the ODE explicit**, solving $\frac{dH}{d\tau} = \alpha \cdot F(H,\tau,u)$ (`dH/dτ = α·F(H,τ,u)`) with a learned vector field.

4. **The output scale $\alpha$ is the stability mechanism**: initialized small (0.1), it bounds the Jacobian and prevents gradient explosion, while remaining learnable if stronger dynamics help.

5. **Control signals $u$ steer the dynamics** continuously throughout integration, potentially enabling smooth, interpretable control over generation.

6. **The adjoint method** provides memory-efficient gradients, enabling deep continuous dynamics without O(depth) memory cost.

7. **The hybrid architecture** keeps discrete layers for early/late processing, using continuous dynamics only where they add value.

The experiment confirms: this works. Gradients flow, training converges, and the architecture is ready for the next step—learning meaningful control dimensions.
