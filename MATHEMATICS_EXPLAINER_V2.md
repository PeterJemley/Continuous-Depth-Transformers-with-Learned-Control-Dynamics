...
8. [The Training Objective: Cross-Entropy Loss](#8-the-training-objective-cross-entropy-loss)
9. [Putting It Together: The Hybrid Architecture](#9-putting-it-together-the-hybrid-architecture)
...
## 8. The Training Objective: Cross-Entropy Loss
...
## 9. Putting It Together: The Hybrid Architecture
...
5. **Control signals $u$ steer the dynamics** continuously throughout integration, potentially enabling smooth, interpretable control over generation. Think: wind altering the current.

6. **The adjoint method** provides memory-efficient gradients, enabling deep continuous dynamics without O(depth) memory cost.

7. **Cross-entropy loss** trains the model to assign high probability to the correct next token.

8. **The hybrid architecture** keeps discrete layers for early/late processing, using continuous dynamics only where they add value.
