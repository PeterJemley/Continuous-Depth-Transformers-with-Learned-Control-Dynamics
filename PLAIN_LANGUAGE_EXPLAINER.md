...
The smooth S-curves from Experiment 3 (the interpolation plot) don't show this. From the outside, steering looks gradual. But the solver reveals hidden structure: **the model isn't smoothly warping one landscape—it's switching between two different landscapes**.

This is like discovering that what looks like one highway on a map is actually two roads at different altitudes.

### The "Decision Point"

We also tracked where the solver works hardest along the flow. Peak effort happens at **τ ≈ 0.67** (about two-thirds through the ODE block). This is where the representation "commits"—the model decides which basin (positive or negative) it's flowing toward.

> **Plain version**: The model does most of its "thinking" about sentiment in the back half of the ODE block. Early stages set up the problem; late stages commit to an answer.
...
The current prototype uses **fixed steps** (Euler method) for training. The solver probing experiments used **adaptive solvers** (dopri5) to reveal geometric structure.

Future directions:
1. **Adaptive Computation**: Use adaptive solvers during inference, allowing the model to "think fast" on easy tokens and "think deep" on hard ones.
2. **Deeper Interpretability**: The two-regime finding suggests we could map the entire control space, identifying basins and boundaries for different semantic attributes.
3. **Multi-dimensional Control**: We validated sentiment only. Formality, creativity, and other axes need richer training objectives.
